...
namespace Enozom.Repositories
{
    public class HotelRepository : IRepository<Hotel>
    {
        ...
        public List<HotelSearchDataShape> SearchByDate(DateTime startDate, DateTime endDate)
        {
            return context.Hotels.Join(
                    context.HotelPrices,
                    hotel => hotel.Id,
                    prices => prices.HotelId,
                    (hotel, prices) => new HotelSearchDataShape { HotelId = hotel.Id, Name = hotel.Name, Date = prices.Date, Price = prices.Price }
